...
#Score calc function
Sg = lambda ldx: sum([fd*math.exp(-0.5-4*t) for (t,fd) in ldx]) #xujun modified
...
class PScore:
    ...
    def readfile(self): #reads the file and returns a vector: each element is a bed_row. 
        ...
        for line in peakf:
            ...
            line = [line[0], int(line[1]), int(line[2]), line[8], float(line[4]),float(line[6])] #xujun modified
            try:
    ...
    def ScoreCalc(self, distance):
        ...
        self.c.execute(sql)
        self.geneInfo = self.c.fetchall() # (0:chrom, 1:name, 2:txStart, 3:txEnd, 4:strand, 5:symbol)
        self.geneInfo.sort()
        self.geneInfo = [list(t) for t in self.geneInfo]

        count = 0
        pvaluecutoff = -10 #xujun modified
        for igene in self.geneInfo:
            ...
            peaksInDistance_FD = [(abs((t[1]+t[2])/2-gTSS)*1.0/distance, t[5]) for t in peaks if t[4]>pvaluecutoff and abs((t[1]+t[2])/2-gTSS) < distance ] #peak pvalue<1e-5, and distance < distance #xujun modified
            #peaksInDistance.sort()
            #if len(peaksInDistance) > 100000000000000000000: # extract no more than 10k peaks
                #peaksInDistance = peaksInDistance[:100000000000000000000]
            #score = sum(math.exp(-0.5-4*t[-1]) for t in peaksInDistance)
            #print peaks
            igene.append(Sg(peaksInDistance_FD)) ##Xujun modified
            count += 1
            if not count % 2000:
...
if __name__ == "__main__":
    main()
    #python /Users/jianma/package/Heatmapr-1.48/bin/CalcScore.py -n test_score -t /Users/jianma/Documents/bioinfor/projects/shuzhen/data1/MACS_result/s_5_1718_peak_peaks.bed -g /Users/jianma/software/hg19.refGene
